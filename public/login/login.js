...
let submitLogin = function () {
    sendThroughWebSocket(JSON.stringify({ type: "login", data: { username: username.value, password: hash(password.value) }, socketType: "dbWS" }))
}
let loginSocket
...
if (localStorage.loginID != "loggedOut" && localStorage.loginID != undefined) {
    ...
    loginSocket = new WebSocket(HOST);
